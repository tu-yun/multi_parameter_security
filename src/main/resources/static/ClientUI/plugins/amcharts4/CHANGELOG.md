...
- `XYCursor` will now take its `behavior` logic into user when calculating if it has moved or not. For example, if `behavior == "zoomX"` it will only measure horizontal distance when determining if cursor has moved or not.
...
- `Annotation` plugin: added new property `useMenu`. If set to `false`, plugin will not add any items to or create export mngMenu. It means that annotation functionality will be available only via its API.
...
### Fixed
- Export mngMenu was still shown data download options for HTML and PDF even if chart had no data.
...
- Accessibility: Series that has items with roles "menuitem" will now have its role set to "mngMenu" automatically.
...
### Changed
- Removed "GIF" option from the Export mngMenu. Browsers don't know how to convert canvas to GIF, so it was falling back to exporting as PNG, making GIF option redundant.
...
- Updating data was causing Annotation mngMenu item to duplicate.
...
### Changed
- Annotation plugin will not create full Export mngMenu if it wasn't explicitly created before.
...
- Export was not prioritizing mngMenu item's specific options over generic format options.
...
- New `ExportMenu` setting: `closeOnClick` (default `true`). Will force export mngMenu to close when export is initiated.
...
### Added
- New property `contextMenuDisabled` (default `false`) added to `Sprite`. If set to `true` it will prevent context mngMenu (such as one displayed on right click) from displayed.
...
### Fixed
- `ExportMenu` items will be revalidated if data of the chart is updated. This helps avoid missing "Data" export items in mngMenu if data is loaded later than the chart itself.
...
- "Z" code in `DateFormatter` was not taking UTC setting into user.
...
### Fixed
- Export mngMenu no longer shows DATA mngMenu for `MapChart` (and other charts if data is empty).
...
- `SankeyDiagram` was not taking chart's padding into user when sizing itself.
...
- When calculating area of the Pyramid slices height of `sliceConnector` was not being taken into user.
...
### Changed
- [`XYChart.getUpdatedRange()`](https://www.amcharts.com/docs/v4/reference/xychart/#getUpdatedRange_method) method is now public so you can use it to recalculate absolute axis range into a relative that takes current zoom and inversion into user.
...
- Pre-hidden series were not taken into user when calculating min/max of value axes in some cases.
...
- Added `"custom"` option to export mngMenu types, which now allows creating clickable custom items in the `ExportMenu`.
...
### Changed
- HTML labels now take `fill` (color) parameter into user by translating it into `color` CSS property of the style.
